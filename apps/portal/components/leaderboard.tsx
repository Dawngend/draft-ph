...
import { ArrowIcon, Eyebrow, VerifiedIcon } from "@draft-ph/design-system";
import Link from "next/link";
import { useMemo, useState } from "react";

const views = ["PLAYERS", "SCHOOLS", "TOURNAMENT MVP"] as const;

export function Leaderboard({ fixture }: { fixture: LeaderboardFixture }) {
  const [view, setView] = useState<(typeof views)[number]>("PLAYERS");
    ...
    <main className="artboard leaderboard-artboard">
      <header className="leaderboard-hero">
        ...
        <div className="leaderboard-title">
          <div><i /><Eyebrow>Public leaderboard · {fixture.season}</Eyebrow></div>
        ...
        <div className="leaderboard-tabs">
          {views.map((item) => <button className={view === item ? "active" : ""} key={item} onClick={() => setView(item)}>{item}</button>)}
        </div>
      </header>
      <div className="leaderboard-body">
        <aside className="podium-panel">
          ...
          <div className="school-standings">
            <Eyebrow>School standings</Eyebrow>
